...
Hybrid modelling has had the aura of the Next Big Thing™ for some years now,[^1] because
...
flowchart TD
    ...
    E["Jax<br/>(Feels like NumPy)"]
    F["Julia<br/>(Feels like Fortran)"]
...
Up to now I haven't even mentioned CPU--GPU data transfer. This can be a significant bottleneck, frequently mitigating any performance gains from executing the neural network on the GPU. Projects like [Oceananigans](https://clima.github.io/OceananigansDocumentation/stable/) [@Silvestri2023] --- the "spiritual successor" to MITgcm --- go much further than translating the code from Fortran to Julia; they completely overhaul the design to target massive parallelism, so that the entire model (not just ML components) run efficiently on GPUs.
